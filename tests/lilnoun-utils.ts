...
import {
  ...
  SeederUpdated,
  Transfer
} from "../generated/lilnoun/lilnoun"
...
export function createLilNoundersDAOUpdatedEvent(
  lilnoundersDAO: Address
): LilNoundersDAOUpdated {
  let lilNoundersDaoUpdatedEvent = changetype<LilNoundersDAOUpdated>(
    newMockEvent()
  )

  lilNoundersDaoUpdatedEvent.parameters = new Array()

  lilNoundersDaoUpdatedEvent.parameters.push(
    new ethereum.EventParam(
      "lilnoundersDAO",
      ethereum.Value.fromAddress(lilnoundersDAO)
    )
  )

  return lilNoundersDaoUpdatedEvent
}
...
export function createNounsDAOUpdatedEvent(nounsDAO: Address): NounsDAOUpdated {
  let nounsDaoUpdatedEvent = changetype<NounsDAOUpdated>(newMockEvent())

  nounsDaoUpdatedEvent.parameters = new Array()

  nounsDaoUpdatedEvent.parameters.push(
    new ethereum.EventParam("nounsDAO", ethereum.Value.fromAddress(nounsDAO))
  )

  return nounsDaoUpdatedEvent
}
